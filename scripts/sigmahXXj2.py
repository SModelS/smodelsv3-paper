...
file_path = f"{base_path}/scripts/sigmahXXj2.txt"
...
with open(file_path, "w") as f:
    ...
    f.write(f"output {base_path}/2mdm_sigmahXXj2nlo\n")
    f.write(f"launch {base_path}/2mdm_sigmahXXj2nlo\n")
    ...
    for j in Sa_values:
        for i in Mh_values:
            ...
            f.write("done\n")
            if i !=  Mh_values[-1] or j != Sa_values[-1] :
                f.write("launch\n")
                
    f.write(f"launch {base_path}/2mdm_sigmahXXj2nlo -i\n")
    f.write(f"print_results --path={base_path}/data/sigmahXXj2nlo.txt --format=short\n")
    f.write("exit\n")