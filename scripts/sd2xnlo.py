...
file_path = f"{base_path}/scripts/Sxxnlo.txt"
...
with open(file_path, "w") as f:
    # Initial setup
    f.write(f"import {base_path}/Feynrules/2MDM/2MDMNLO -modelname\n") 
    f.write("define p = g u c d s b u~ c~ d~ s~ b~\n")
    f.write("generate p  p > h2 [noborn=QCD] > chi chi @1\n")
    f.write(f"output Sxxnlo\n")
    f.write(f"launch Sxxnlo\n")
    ...
    f.write(f"launch Sxxnlo -i\n")
    f.write(f"print_results --path={base_path}/data/Sxxnlo.txt --format=short\n")
    f.write("exit\n")