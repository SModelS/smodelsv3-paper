...
file_path = f"{base_path}/scripts/sigmahNewnlo.txt"

Mh_values = [140, 300, 500, 700, 900, 1100, 1300, 1500, 1700, 1900, 2100, 2400]
ychi_values = [1,1.5]
...
with open(file_path, "w") as f:
    ...
    f.write(f"output {base_path}/2mdm_sigmahNewnlo\n")
    f.write(f"launch {base_path}/2mdm_sigmahNewnlo\n")
    ...
    f.write("set param_card MSd 140\n")

    for j in ychi_values:
        for i in Mh_values:
            f.write(f"set param_card Mh {i}\n")
            f.write(f"set param_card Wh auto \n")
            f.write(f"set param_card ychi {j}\n")
            f.write("done\n")
            if i !=  Mh_values[-1] or j != ychi_values[-1] :
                f.write("launch\n")
                
    f.write(f"launch {base_path}/2mdm_sigmahNewnlo -i\n")
    f.write(f"print_results --path={base_path}/data/sigmahNewnlo.txt --format=short\n")
    f.write("exit\n")